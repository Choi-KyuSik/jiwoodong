            ...
            <div id="s_um_delete_content" class="tab_menu s_content" style="display: block;">
				...
				<div id="pagingBox">
					...
					<c:if test="${startPage > 1 }">
						<li class="page-item"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${startPage-1 }">Previous</a></li>
					</c:if>
					<c:if test="${startPage <= 1 }">
						<li class="page-item disabled"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${startPage-1 }">Previous</a></li>
					</c:if>
					<c:forEach step="1" begin="${startPage }" end="${ endPage}" var="idx">
						<c:if test="${idx eq currentPage }">
						<li class="page-item active"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${idx }">${idx }</a></li>
						</c:if>
						<c:if test="${idx ne currentPage }">
						<li class="page-item"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${idx }">${idx }</a></li>
						</c:if>
					</c:forEach>
					<!-- endPage에서 +1일 때 -->
					<c:if test="${endPage < pageCnt }">
						<li class="page-item"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${endPage+1 }">Next</a></li>
					</c:if>
					<c:if test="${endPage >= pageCnt }">
						<li class="page-item disabled"><a class="page-link" href="AdminUsInfoDeleteList?pageNum=${endPage+1 }">Next</a></li>
					</c:if>
					</ul>
				</div>
            </div>
